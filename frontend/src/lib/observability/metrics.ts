...
collectDefaultMetrics({ prefix: 'studio_' });

// Define metrics to match core/metrics.py
export const REQUESTS_TOTAL = new Counter({
    name: 'studio_requests_total',
    help: 'Total number of requests processed',
    labelNames: ['method', 'endpoint', 'status']
});

export const PROCESSING_DURATION = new Histogram({
    name: 'studio_processing_duration_seconds',
...
export const ACTIVE_REQUESTS = new Gauge({
    name: 'studio_active_requests',
    help: 'Number of requests currently being processed'
});

export const SYSTEM_CPU_USAGE = new Gauge({
    name: 'studio_system_cpu_usage_percent',
    help: 'System CPU usage percentage'
});

export const SYSTEM_RAM_USAGE = new Gauge({
    name: 'studio_system_ram_usage_bytes',
