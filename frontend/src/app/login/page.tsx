...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
      // Call Next.js API route (proxied to backend)
          ...
          <div className="mt-8 pt-6 border-t border-border">
            <div className="text-center">
              ...
              <div className="space-y-2 text-xs text-muted-foreground">
                <div className="bg-muted/50 rounded-lg p-3 cursor-pointer hover:bg-muted transition-colors" onClick={() => { setEmail('admin@example.com'); setPassword('password123'); }}>
                  <div className="font-medium text-card-foreground mb-1">Administrator</div>
                  <div>admin@example.com / password123</div>
                </div>
                <div className="bg-muted/50 rounded-lg p-3 cursor-pointer hover:bg-muted transition-colors" onClick={() => { setEmail('manager@example.com'); setPassword('password123'); }}>
                  <div className="font-medium text-card-foreground mb-1">Manager</div>
                  <div>manager@example.com / password123</div>
                </div>
                <div className="bg-muted/50 rounded-lg p-3 cursor-pointer hover:bg-muted transition-colors" onClick={() => { setEmail('auditor@example.com'); setPassword('password123'); }}>
                  <div className="font-medium text-card-foreground mb-1">Auditor</div>
                  <div>auditor@example.com / password123</div>
                </div>
                <div className="bg-muted/50 rounded-lg p-3 cursor-pointer hover:bg-muted transition-colors" onClick={() => { setEmail('customer@example.com'); setPassword('password123'); }}>
                  <div className="font-medium text-card-foreground mb-1">Customer</div>
                  <div>customer@example.com / password123</div>
                </div>
              </div>
            </div>
          </div>
