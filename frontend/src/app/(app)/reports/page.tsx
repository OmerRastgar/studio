...
function ReportsContent() {
  ...
  const isViewOnly = role === 'manager' || role === 'admin';
  ...
  const { trackActivity, sessionSeconds } = useActivityTracker({
    ...
    isDisabled: isViewOnly, // Generalized disability check
  ...
  useEffect(() => {
    const fetchProjectDetails = async () => {
      ...
      try {
        ...
        // Removed useEffect set state for isViewOnly/isAdmin as they are now derived
        ...
        if (typeof data.data.isReviewer !== 'undefined') {
          ...
          // Fallback for reviewer check if backend doesn't explicit send it
          // This might happen if user is manually assigned reviewer
    ...
    fetchProjectDetails();
  }, [selectedProject, token, apiBase, toast, user]);
  ...
  const resolveFlag = async (rowId: string | null | undefined) => {
    if (!rowId) return;
    trackActivity('review');
  ...
  return (
    <>
      <EvidencePicker
        ...
        onOpenChange={(open) => setPickerState(prev => ({ ...prev, isOpen: open, rowId: prev.rowId || null }))}
                      ...
                      <TableRow key={row.id} className={cn("align-top transition-colors duration-500",
                        ...
                        <TableCell>
                          <div className='flex items-start gap-2'>
                            {(row.isFlagged || row.isResolved) && (
                              <button onClick={() => !isViewOnly && handleFlagButtonClick(row)} disabled={isViewOnly} className={isViewOnly ? "cursor-default" : ""}>
                                {row.isFlagged && <Flag className={cn("h-4 w-4 mt-2 text-orange-500", !isViewOnly && "cursor-pointer")} />}
                                {row.isResolved && <CheckCircle className={cn("h-4 w-4 mt-2 text-green-500", !isViewOnly && "cursor-pointer")} />}
                              </button>
                            )}
                            <Textarea
                              value={row.control}
                          ...
                          {/* Display Tags - Moved to Control Column */}
                          {row.tags && row.tags.length > 0 && (
                            <div className="flex flex-wrap gap-1 mt-2">
                              {row.tags.map((tag, idx) => (
                                <Badge key={`${row.id}-tag-${idx}`} variant="outline" className="text-xs py-0 h-5">
                                  {tag}
                                </Badge>
                              ))}
                            </div>
                          ...
                          <div onClick={() => {
                            if (isViewOnly) {
                              toast({
                                ...
                                description: "Only assigned Auditors can edit observations."
                              });
                            } else if (isReviewer && !(user?.id && currentProjectDetails?.auditor?.id === user.id)) {
                              toast({
                                variant: "destructive",
                                title: "Permission Denied",
                                description: "Reviewers cannot edit observations, only flag them."
                              });
                            }
                          }}>
                            <Textarea
                              ...
                              readOnly={isViewOnly || (isReviewer && !(user?.id && currentProjectDetails?.auditor?.id === user.id))}
                              disabled={isViewOnly || (isReviewer && !(user?.id && currentProjectDetails?.auditor?.id === user.id))}
                              className={cn(
                                "min-h-[100px]",
                                (isViewOnly || isReviewer) && "cursor-not-allowed opacity-80"
                              )}
                            />
                          </div>
                        ...
                        <TableCell>
                          <div className="flex gap-1 flex-wrap items-center">
                            {row.evidence.length > 0 ? row.evidence.map((ev) => (
                              <Badge key={ev.id} variant="secondary" className="flex items-center gap-1 pr-1">
                                <a href={`/evidence/${ev.id}/view`} target="_blank" rel="noopener noreferrer" className="text-blue-600 hover:underline truncate block">
                                  {ev.fileName}
                                </a>
                                {!isViewOnly && (!isReviewer || (user?.id && currentProjectDetails?.auditor?.id === user.id)) && (
                                  <button
                                    onClick={() => handleUnlinkEvidence(row.id, ev.id)}
                        ...
                        <TableCell className="text-right">
                          <div className='flex flex-col gap-2 items-end'>
                            ...
                            <div className="flex items-center gap-2">
                              ...
                              />
                              <button onClick={() => handleFlagButtonClick(row)} className="p-2" disabled={isViewOnly}>
                                <Flag className={cn("h-4 w-4", row.isFlagged ? "text-orange-500 fill-orange-500" : "text-muted-foreground", isViewOnly ? "opacity-50 cursor-not-allowed" : "")} />
                              </button>
                            </div>
                          </div>
                        </TableCell>
