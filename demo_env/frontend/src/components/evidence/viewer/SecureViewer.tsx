...
export function SecureViewer({ evidenceId, objectKey, filename, contentType, token, onClose }: SecureViewerProps) {
    const [viewType, setViewType] = useState<'visual' | 'data' | 'audio' | 'unknown'>('visual');
    ...
    useEffect(() => {
        ...
        // We trust the backend 'viewer' service to handle (or error on) the file.
        // This supports .txt, .config, .log, .docx, .odt, .xlsx, .csv, Dockerfile, etc.
        else {
            setViewType('visual');
        }
