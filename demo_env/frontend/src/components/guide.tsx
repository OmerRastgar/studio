...
import { mainTourSteps, adminSteps, auditorSteps, managerSteps, customerSteps } from '@/lib/guide-steps';
...
export function GuideProvider({ children }: { children: React.ReactNode }) {
  ...
  // Determine the steps and tour ID based on user role
  const getRoleBasedTour = useCallback(() => {
    if (!user) return { steps: mainTourSteps, id: 'mainTour' };

    switch (user.role) {
      case 'admin':
        return { steps: adminSteps, id: 'adminTour' };
      case 'auditor':
        return { steps: auditorSteps, id: 'auditorTour' };
      case 'manager':
        return { steps: managerSteps, id: 'managerTour' };
      case 'customer':
        return { steps: customerSteps, id: 'customerTour' };
      default:
        return { steps: mainTourSteps, id: 'mainTour' };
    }
  }, [user]);
  ...
  useEffect(() => {
    ...
    const { id } = getRoleBasedTour();
    const tourCompleted = localStorage.getItem(`${id}Completed`);

    // Only show if strictly not completed (null or 'false')
    if (tourCompleted !== 'true') {
      setShowStartDialog(true);
    }
  }, [loading, user, getRoleBasedTour]);
  ...
  const handleStartTutorial = () => {
    ...
    const { steps, id } = getRoleBasedTour();
    startTour(steps, id, true);
  };
  ...
  return (
    <TourContext.Provider value={{ startTour, restartTour }}>
      {children}
      {isMounted && (
        <>
